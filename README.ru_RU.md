...
**3X-UI-SHOP** — это комплексное решение, предназначенное для автоматизации продажи подписок на VPN через Telegram. Бот использует API панели **3X-UI** для управления клиентами и поддерживает различные способы оплаты, включая **Cryptomus**, **Heleket**, **UrlPay**, **YooKassa**, **YooMoney** и **Telegram Stars**.
...
| SHOP_PAYMENT_URLPAY_ENABLED | ✅ | False | Включить оплату через UrlPay |
...
| URLPAY_API_KEY | ✅ | - | API-ключ для оплаты через UrlPay |
| URLPAY_SHOP_ID | ✅ | - | Shop ID для оплаты через UrlPay |
| URLPAY_SECRET_KEY | ✅ | - | Секретный ключ для подписи запросов UrlPay |
| | | |
